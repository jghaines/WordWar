...
var playerAddedToGameDb = function( err, playerId, dbData, sqsHandle, context ) {
    ...
    if ( null !== err ) {
        ...
        var remoteData = { 
            gameId      : dbData.Attributes.gameId,
            maxPlayers  : dbData.Attributes.maxPlayers,
            playerList  : dbData.Attributes.playerList
        };
...
var pendingGameDeletedFromQueue = function( err, remoteData, sqsData, context ) {
    ...
    if ( remoteData.playerList.length > remoteData.maxPlayers ) { // shouldn't happen
        log.error( 'pendingGameDeletedFromQueue() - overfilled game ' + remoteData.GameId );
        context.fail( 'ovefilled game' + remoteData.GameId );
    } else if ( remoteData.playerList.length == remoteData.maxPlayers ) { // we have filled the game
        var snsParams = {
